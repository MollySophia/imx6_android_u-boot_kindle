...
#define PRINT_TO_BUILD_CONSOLE

#ifdef PRINT_TO_BUILD_CONSOLE
...
static void print_hdr_v2(struct imx_header *imx_hdr)
{
	...
	if (!plugin) {
		...
		if (fhdr_v2->csf && (imximage_ivt_offset != UNDEFINED) &&
			...
			uint32_t length = hdr_v2->boot_data.size - imximage_ivt_offset - imximage_csf_size;
			length = (length + 4095) & ~4095; // Round 4K
			printf("HAB Blocks:   %08x %08x %08x\n",
			       (uint32_t)fhdr_v2->self, 0, length);
		}
	} else {
		imx_header_v2_t *next_hdr_v2;
		flash_header_v2_t *next_fhdr_v2;
				...
				struct image_tool_params *params)
{
	...
	*header_size_ptr = ROUND((sbuf->st_size + imximage_ivt_offset), 4096);
	...
	if (csf_ptr && imximage_csf_size) {
		...
		*csf_ptr = *ivt_ptr + ((*csf_ptr - *ivt_ptr + 4095) & ~4095); // 4K alignment
	}
}
