...
#if defined(CONFIG_FASTBOOT) && defined(CONFIG_ANDROID_RECOVERY)
#include <recovery.h>
#endif
...
/*
 * check Android related boot modes [aboot_mode]
 * SNVS_LPGPR[3-0] bits are used to store aboot_mode
 * Read aboot_mode and clear register aboot_mode bits.
 */
...
#endif
	reg = writel(reg, SNVS_BASE_ADDR + SNVS_LPGPR);

	return aboot_mode;
}

/*
 * set Android related boot mode.
 * SNVS_LPGPR[3-0] bits are used to store aboot_mode
 */
void imx_set_aboot_mode(u32 mode)
{
	...
	writel(mode, SNVS_BASE_ADDR + SNVS_LPGPR);
}
...
int recovery_check_and_clean_flag(void)
{
	...
	return (aboot_mode == ANDROID_BOOT_RECOVERY)?1:0;
}
