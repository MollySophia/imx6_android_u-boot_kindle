...
static int do_idme(char *response, int flag, int argc, char * const argv[]);
#else
static int do_idme(int flag, int argc, char * const argv[]);
#endif
...
#if !defined(UFBL_TESTS) && !defined(UFBL_PROJ_ABC) && !defined(UFBL_PROJ_ABC)
...
{
	char *token;
	char ver[32], value[65];	/* 65 is good enough for now, nothing is over 64 */
...
#endif
{
	...
	if (argc == 1 || strncmp(argv[1], "?", 1) == 0) {
		if (argc == 3) {
			char buf[1024] = {0};
			int len = snprintf(buf, sizeof(buf), "%s: ", argv[2]);
			if (len >= sizeof(buf))
				len = sizeof(buf) - 1;
			if (idme_get_var) {
