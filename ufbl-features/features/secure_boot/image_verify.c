...
{
	enum safe_verify_response resp =
		amzn_image_verify_safe(image, signature, image_size, handler);
	return (resp & (AMZN_IMAGE_SIGNED | AMZN_IMAGE_DEVSIGNED_ENG))
		? 1 : 0;
}

/*
 * Returns a bit mask telling you what kind of verification passed, if any
 * Expects a pointer to the start of image and pointer to start of sig
 */
enum safe_verify_response amzn_image_verify_safe(const void *image,
	...
	unsigned int image_size, meta_data_handler handler)
{
	enum safe_verify_response auth = AMZN_IMAGE_BAD;
	char *digest = NULL;
	...
	if (amzn_verify_image(AMZN_PRODUCTION_CERT, digest,
					signature, handler)) {
		/* Image is not prod signed. Is it engineering signed? */
		if (amzn_verify_image(AMZN_ENGINEERING_CERT, digest,
					signature, handler)) {
			dprintf(ALWAYS, "Image authentication FAILED\n");
			/* Image is not even dev signed.
			 * Some loaders may allow it.*/
			if (amzn_target_device_type() ==
			    AMZN_ENGINEERING_DEVICE)
				auth |= AMZN_IMAGE_UNSIGNED_ENG;
#if defined(UFBL_FEATURE_UNLOCK)
			if (amzn_target_is_unlocked())
				auth |= AMZN_IMAGE_UNSIGNED_UNLOCKED;
#endif
		} else {
			dprintf(ALWAYS,
				"Image AUTHENTICATED with ENGINEERING certificate\n");
			/* Image is dev signed */
			auth |= AMZN_IMAGE_DEVSIGNED;
			if (amzn_target_device_type() ==
			    AMZN_ENGINEERING_DEVICE)
				auth |= AMZN_IMAGE_DEVSIGNED_ENG;
#if defined(UFBL_FEATURE_UNLOCK)
			if (amzn_target_is_unlocked())
				auth |= AMZN_IMAGE_DEVSIGNED_UNLOCKED;
#endif
		}
	} else {
		dprintf(ALWAYS,
			"Image AUTHENTICATED with PRODUCTION certificate\n");
		auth = AMZN_IMAGE_SIGNED;
	}
