...
#if !defined(UFBL_PROJ_ABC)
#if !defined(NON_ANCIENT_UBOOT_LK)
static char *skip_spaces(const char *arg) {
	while(*cursor && isblank(*arg))
		arg++;
	...
	switch (flags_cmd) {
		case FLAGS_CMD_PRINT: {
			if (FLAGS_TYPE_FOS == flags_type) {
				...
				fos_flags_to_str(cur_flags, buffer, size_needed);
				dprintf(SPEW, "%s is currently 0x%llx\n", target_flags, cur_flags);
				snprintf(response, response_length, "fos_flags: %s", buffer);
			} else {
