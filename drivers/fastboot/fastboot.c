...
extern void draw_device_recovery_screen(void);
#endif
...
#ifdef CONFIG_CMD_IDME
	memcpy(serial_number, get_board_serial(), SERIAL_NUM_LEN);
...
#ifdef CONFIG_LAB126_FASTBOOT_GPT
	/* Load fastboot partitions from GPT on disk */
	setup_fastboot_partitions_from_gpt();
#endif
}
...
static int _fastboot_parts_load_from_ptable(void)
{
	...
	ptable[PTN_MBR_INDEX].start = EANAB_MBR_OFFSET / dev_desc->blksz;
	ptable[PTN_MBR_INDEX].length = EANAB_MBR_SIZE / dev_desc->blksz;
