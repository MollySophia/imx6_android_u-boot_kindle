...
extern void setup_recovery_bmp(void);
extern void setup_eol_bmp(void);
extern void setup_bmp_image(void *image, ulong ilen);
extern bool pmic_low_battery(void);
extern bool device_powered_off(void);
void draw_device_eol_screen(void);
#endif
...
static void epdc_init_settings(void)
{
	...
	/* Enables stride to allow update buffer not 64 bits aligned */
	reg_val = panel_info.vl_col;
	REG_WR(EPDC_BASE, EPDC_UPD_STRIDE, reg_val);
	...
	int i=0;
	for (i = 0; i < 5; i++)
		REG_WR(EPDC_BASE, EPDC_AUTOWV_LUT,
...
{
	lcd_base = (void *)(uintptr_t)gd->fb_base;

	/* if one wish to draw a boot up default splash, do it in this function */
	draw_device_eol_screen();
}
...
#ifdef CONFIG_FASTBOOT_DRAW_SPLASH
/*
 * draw device recovery mode splash
 */
void draw_device_recovery_screen(void)
{
	epdc_power_on();

	/* draw blank in white to start clean */
	...
	/* power off the panel to save power */
	epdc_power_off();
}

/*
 * draw early sign of life splash
 */
void draw_device_eol_screen(void)
{
	/* only draw if the device was powered off (shipmode) */
	if (!device_powered_off())
		return;
...
/*
 * draw low battery splash
 */
void draw_device_critbatt_screen(void)
{
	epdc_power_on();
...
#endif /* CONFIG_FASTBOOT_DRAW_SPLASH */
