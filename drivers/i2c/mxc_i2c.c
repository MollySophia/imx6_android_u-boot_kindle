...
static int i2c_smb_init_transfer_(struct mxc_i2c_regs *i2c_regs,
                uchar chip, uchar rw)
{
        unsigned int temp;
        int ret;
        ...
        /* write slave address */
        ret = tx_byte(i2c_regs, chip << 1 | rw);
        if (ret < 0)
                return ret;
...
static int i2c_smb_init_transfer(struct mxc_i2c_regs *i2c_regs,
                uchar chip, uchar rw)
{
        int retry;
        int ret;
        for (retry = 0; retry < 3; retry++) {
                ret = i2c_smb_init_transfer_(i2c_regs, chip, rw);
                if (ret >= 0)
                        return 0;
                i2c_imx_stop(i2c_regs);
                if (ret == -ENODEV)
...
int bus_i2c_smb_write(void *base, uchar chip, const uchar *buf, int len)
{
        int ret;
        int i;
        struct mxc_i2c_regs *i2c_regs = (struct mxc_i2c_regs *)base;

        ret = i2c_smb_init_transfer(i2c_regs, chip, 0x0);
        if (ret < 0)
                return ret;
...
int bus_i2c_smb_read(void *base, uchar chip, uchar *buf, int len)
{
        int ret;
        unsigned int temp;
        int i;
        ...
        ret = i2c_smb_init_transfer(i2c_regs, chip, 0x1);
        if (ret < 0)
                return ret;
