...
#ifdef CONFIG_LAB126_FASTBOOT_GPT

/**
 * setup_fastboot_partitions_from_gpt(): Loads fastboot partitions from GPT
 *
 * Description: Reads the GPT and sets up the fastboot flash partitions. This
 * code is very similar to the print_part_efi() method.
 */
void setup_fastboot_partitions_from_gpt() {
	block_dev_desc_t *dev_desc = mmc_get_dev(CONFIG_MMC_DEFAULT_DEV);
	ALLOC_CACHE_ALIGN_BUFFER_PAD(gpt_header, gpt_head, 1, dev_desc->blksz);
	gpt_entry *gpt_pte = NULL;
	int i = 0;
	char uuid[37];
	...
	for (i = 0; i < le32_to_cpu(gpt_head->num_partition_entries); i++) {
		...
		/* The ending LBA is inclusive, to calculate size, add 1 to it */
		ptentry.start = (unsigned int) le64_to_cpu(gpt_pte[i].starting_lba);
		ptentry.length = (unsigned int) le64_to_cpu(gpt_pte[i].ending_lba) + 1 - ptentry.start;
		strncpy(ptentry.name, print_efiname(&gpt_pte[i]), 16);
		fastboot_flash_add_ptn(&ptentry);
	}
}
#endif //CONFIG_LAB126_FASTBOOT_GPT
