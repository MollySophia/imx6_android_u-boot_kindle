...
export SRCTREE OBJTREE GET_LOCAL_DIR KBUILD_LOCAL_DIR_FUNC KBUILD_LOCAL_DIR

UFBL_ENABLE     := yes
UFBL_PATH       := $(srctree)/ufbl-features
...
ifeq ($(ENABLE_UFBL_SECURE_BOOT),true)
KBUILD_CFLAGS += -DENABLE_UFBL_SECURE_BOOT -DUFBL_CONFIG_KBUILD -DCONFIG_UFBL
endif
...
UBOOTINCLUDE += -I$(UFBL_PATH)/include \
				-I$(UFBL_PATH)/features/secure_boot
...
%.imx: %.bin
	...
	dd if=u-boot.imx of=u-boot.android.bin bs=1024 seek=1
	@echo cp u-boot.android.bin u-boot.imx
	@cp u-boot.android.bin u-boot.imx
