...
#ifdef __ARM_ARCH__
        ERROR: Please fix security FIREOS-25947. Code below calls PEM_read_RSAPrivateKey()
        with a null password. A private key and/or secret information is not allowed to be
        saved to disk files without protection. Thus this code is allowed to be used on a
        protected server machines, not on any device. If this comment causes compile error,
        please revisit the code and implement password protection then remove this comment.
#endif //ARM_ARCH__
